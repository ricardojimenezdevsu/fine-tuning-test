...
def gpt3_completion(prompt, engine='text-davinci-003', temp=0.7, top_p=1.0,  tokens=300, freq_pen=0.0, pres_pen=0.0, stop=['']):
    max_retry = 5
    retry = 0
    while True:
        try:
...
if __name__ == '__main__':
    ...
    for book in books:
        ...
        for chunk in chunks:
            count += 1
            prompt = open_file('p_summary_detailed.txt').replace('<<TEXT>>', open_file('chunks/%s' % chunk))

            prompt = prompt.encode(encoding='ASCII', errors='ignore').decode()
            print(prompt)
            response = gpt3_completion(prompt)
            print(name, chunk, response)
            save_file(response, 'chunks_summarized/%s' % chunk)
            if count >= 1:
                break