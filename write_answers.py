...
def gpt3_completion(prompt, engine='text-davinci-003', temp=0.7, top_p=1.0,  tokens=1000, freq_pen=0.0, pres_pen=0.0, stop=['<<STOP>>']):
    max_retry = 5
    retry = 0
    while True:
        try:
...
if __name__ == '__main__':
    ...
    for book in books:
        ...
        for chunk in chunks:
            count += 1
            prompt = open_file('prompt_answers.txt').replace('<<CONTEXT>>', open_file('chunks/%s' % chunk))
            prompt = prompt.replace('<<QUESTIONS>>', open_file('questions/%s' % chunk))
            prompt = prompt.encode(encoding='ASCII', errors='ignore').decode()

            response = gpt3_completion(prompt)
            print(name, chunk, response)
            save_file(response, 'answers/%s' % chunk)
            # if count >= 2:
            #     break